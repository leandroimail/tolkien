...
| Google Antigravity | `AGENTS.md`, `GEMINI.md`, `.agents/rules/*.md` | `.agents/skills/*/SKILL.md` (progressive disclosure) | Slash commands (`/<skill-name>`, `/academic-orchestrator`) | `.agents/hooks.json` (lifecycle hooks), `.agents/workflows/*.md` | Orchestrates via canonical `.agents/agents/*.md` & skills |
...
| Google Antigravity | Canonical `.agents/agents/*.md` + skills invocation | IDE tool control (run_command, file tools) + lifecycle hooks | `.agents/mcp_config.json` or IDE MCP settings | Yes — canonical `.agents/skills` with progressive disclosure |
