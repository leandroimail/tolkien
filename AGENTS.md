...
The **Academic Article Production Multi-Agent System (tolkien)** is a powerful multi-agent framework designed to support the complete production cycle of academic and scientific papers. It is fully compatible with **Google Antigravity**, **Claude Code** (CLI), **OpenCode**, and **OpenAI Codex**, using a standardized structure of specialized agents and atomic skills.
...
tolkien uses a canonical-first architecture supporting multiple AI agent environments:
...
- **`.claude/`**: Dedicated mirror for **Claude Code (CLI)** (`.claude/skills/`, `.claude/agents/`, `.claude/settings.json`, and root `CLAUDE.md`).
- **`.codex/`**: OpenAI Codex harness configuration (`.codex/hooks.json`) and agent descriptors (`.codex/agents/*.toml`).
- **`.opencode/`**: OpenCode harness plugins (`.opencode/plugins/format-validator.js`) and agent descriptors (`.opencode/agents/*.md`).
...
├── AGENTS.md                   ← Canonical rules & system documentation (this file)
